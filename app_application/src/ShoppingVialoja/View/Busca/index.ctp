		...
		<div class="row">
			...
			<section id="center_column" class="col-md-12">
				<div id="breadcrumb" class="clearfix">
					...
					<div id="breadcrumb" class="clearfix">
	                    ...
	                    <div class="breadcrumb clearfix">
	                        <a class="home" href="//<?php echo env('HTTP_HOST'); ?>" title="Voltar para a P&aacute;gina Inicial"><i class="fa fa-home"></i></a>
				...
				<h1 
                    ...
                    <?php elseif (count($res_produto) == 1 ): ?>
                    	1 resultado foi encontrado.
                    <?php else: ?>
                    	<?php echo count($res_produto); ?> resultados foram encontrados.
                    <?php endif ?>
                ...
                <p class="alert alert-warning">
                    Sua pesquisa não teve resultados&nbsp;"<?php echo \Lib\Tools::getValue('search_query'); ?>"
                </p>
								...
								<form id="productsSortForm" action="<?php echo \Lib\Tools::getUrl(); ?>" class="productsSortForm form-horizontal pull-left hidden-xs ">
									<div class="select">
										...
										<select class="form-control selectProductSort" id="selectProductSort">
											...
											if (\Lib\Tools::getValue('orderby') !='') {
												$orderby = \Lib\Tools::getValue('orderby');
											}

											if (\Lib\Tools::getValue('orderway') !='') {
												$orderway = \Lib\Tools::getValue('orderway');
											}
											
											if ($orderby == 'posicao' && $orderway == 'asc' ) {
												echo '<option value="posicao:asc" selected="selected">--</option>';
											} else {
												echo '<option value="posicao:asc" selected="selected">--</option>';
											}

											if ($orderby == 'preco' && $orderway == 'asc' ) {
												echo '<option value="preco:asc" selected="selected">Mais Barato</option>';
											} else {
												echo '<option value="preco:asc">Mais Barato</option>';
											}

											if ($orderby == 'preco' && $orderway == 'desc' ) {
												echo '<option value="preco:desc" selected="selected">Mais Caro</option>';
											} else {
												echo '<option value="preco:desc">Mais Caro</option>';
											}

											if ($orderby == 'nome' && $orderway == 'asc' ) {
												echo '<option value="nome:asc" selected="selected">De A a Z</option>';
											} else {
												echo '<option value="nome:asc">De A a Z</option>';
											}

											if ($orderby == 'nome' && $orderway == 'desc' ) {
												echo '<option value="nome:desc" selected="selected">De Z a A</option>';
											} else {
												echo '<option value="nome:desc">De Z a A</option>';
											}

											if ($orderby == 'quantidade' && $orderway == 'desc' ) {
												echo '<option value="quantidade:desc" selected="selected">Disponivel</option>';
											} else {
												echo '<option value="quantidade:desc">Disponivel</option>';
											}

											if ($orderby == 'referencia' && $orderway == 'asc' ) {
												echo '<option value="referencia:asc" selected="selected">Referência: Menor primeiro</option>';
											} else {
												echo '<option value="referencia:asc">Referência: Menor primeiro</option>';
											}

											if ($orderby == 'referencia' && $orderway == 'desc' ) {
												echo '<option value="referencia:desc" selected="selected">Referência: Maior primeiro</option>';
											} else {
												echo '<option value="referencia:desc">Referência: Maior primeiro</option>';
											}

											?>

											<input type="hidden" name="q" value="<?php echo \Lib\Tools::getValue('q'); ?>" />
				...
				<div  class="nomargin product_list grid row ">
					...
					$dominio = $produto['ShopDominio']['dominio'];
					$ssl_ativo = $produto['ShopDominio']['ssl_ativo'];
					$preferencia_url = $produto['Shop']['preferencia_url_dominio'];
					...
					if ($subdominio_plataforma == 'True') {
						if ($ssl_ativo=='True')
							FULL_BASE = 'https://'. $dominio;
						else
							FULL_BASE = 'http://'. $dominio;
					} else {
						...
						if ($preferencia_url=='off_www') {
							...
							if ($ssl_ativo=='True')
								FULL_BASE = 'https://www.'. $dominio;
							else
								FULL_BASE = 'http://www.'. $dominio;
						}
						...
						<div class="product-container text-center product-block" itemscope itemtype="http://schema.org/Product">
							<div class="left-block">
								<div class="product-image-container image" align="center">
									...
									<a class="product_img_link"	href="<?php printf('%s/p/%s/%d/', FULL_BASE, \Lib\Tools::slug($nome), $id_produto; ?>" title="<?php echo $produto['ShopProduto']['nome']; ?>" itemprop="url">
									...
									<?php /* ?>	
									<a class="quick-view btn-outline-inverse btn" href="<?php printf('%s/p/%s/%d/', FULL_BASE, \Lib\Tools::slug($nome), $id_produto; ?>" rel="<?php printf('%s/p/%s/%d/', FULL_BASE, \Lib\Tools::slug($nome), $id_produto; ?>" title="Visualização rápida">
							...
							<div class="right-block">
								<div class="product-meta">
									...
									<h5 itemprop="name" class="name">
										<a class="product-name" href="<?php printf('%s/p/%s/%d/', FULL_BASE, \Lib\Tools::slug($nome), $id_produto; ?>" title="<?php echo $produto['ShopProduto']['nome']; ?>" itemprop="url">
									...
									<div class="functional-buttons clearfix">
										...
										<div class="compare">
											<a class="add_to_compare compare btn btn-outline-inverse" href="<?php printf('%s/p/%s/%d/', FULL_BASE, \Lib\Tools::slug($nome), $id_produto; ?>" data-id-product="1" title="Adicionar para comparar">
